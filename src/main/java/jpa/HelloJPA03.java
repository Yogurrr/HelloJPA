...
//            jpql = "select distinct count(distinct jobid) from Employee e";
...
//            List<Employee> pemps = em.createQuery(jpql, Employees.class).setFirstResult(30).setMaxResults(15).getResultList();

//            for(Employee pemp : pemps) System.out.println(pemp);
...
//            for(Object[] item : items) System.out.println(item[0] + "/" + item[1] + "/" + item[2] + "/" + item[3]);


            // 부서번호가 60번인 사원들의 이름, 직책, 부서명 조회 : join
//            jpql = " select e.fname, e.jobid, e.deptid from Employee e inner join e.department d where e.deptid = 60 ";
//            List<Object[]> items = em.createQuery(jpql).getResultList();
...
//            jpql = " select empid, hdate from Employee e where deptid = (select deptid from Department d where dname = 'IT') ";
//            List<Object[]> items = em.createQuery(jpql).getResultList();
